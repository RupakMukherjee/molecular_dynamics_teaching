...
implicit none
integer N,i,j,p
real*8 Lx,Ly,Lz,Vxmax,Vymax,Vzmax,tmax,dt,svx,svy,svz,fx,fy,fz,scl,KE,Temp,k,t,xdiff,ydiff,zdiff,r,tau
  ...
  az(i) = 0.0d0
    do j = 1,N
      if (i .ne. j) then
        ...
        fx = xdiff*(1.0d0+k*r)*exp(-k*r)/(r*r*r)
        fy = ydiff*(1.0d0+k*r)*exp(-k*r)/(r*r*r)
        fz = zdiff*(1.0d0+k*r)*exp(-k*r)/(r*r*r)
        ax(i) = ax(i) + fx
        ay(i) = ay(i) + fy
        az(i) = az(i) + fz
      endif
